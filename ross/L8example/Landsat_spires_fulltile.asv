...
%files
r0dir=fullfile(pwd,'data','R0',pr); %snow/ice minima background, p42r34 20201014
% rdir=fullfile(pwd,'R'); %snow covered scene, p42r34 20160426
demfile=fullfile(pwd,'data','DEM',strcat(pr,'_dem.mat')); % DEM for p42r34 - extraneous 
%if terrain correction set to false & el_cutoff = 0 m
Ffile=fullfile(pwd,'data','Ffile','lut_oli_b1to7_3um_dust.mat'); % look up tables
%Mie-RT calcs for snow for L8 bands 1-7 w/ 3 um dust
CCfile=fullfile(pwd,'data','CC',strcat(pr,'_cc.mat')); % canopy cover percent file, NLCD
WaterMaskfile=fullfile(pwd,'data','watermask',strcat(pr,'_watermask.mat')); %watermask file, NLCD
CloudMaskfile=fullfile(pwd,'data','cloudmask',strcat(pr,'_cloudmask.mat')); %RTP added
fIcefile=fullfile(pwd,'data','fice',strcat(pr,'_fice.mat')); %fractional ice, derived from
